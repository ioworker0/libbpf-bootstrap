/* captrace BPF program for kernel 4.18: uses perf_event_output instead of ring_buffer. */
#include "vmlinux.h"
...
/* Perf event array: key = CPU, value = fd (set by user space). 4.18 compatible. */
struct {
...
static __always_inline int record_cap(void *ctx, int cap, int stack_id)
{
